# -*- encoding: utf-8 -*-
# @Introduce  : 
# @File       : RibonanzaNet.py
# @Author     : ryrl
# @Email      : ryrl970311@gmail.com
# @Time       : 2025/03/07 05:26
# @Description: https://www.kaggle.com/datasets/shujun717/ribonanzanet2d-final
...
class Mish(nn.Module):  # a smooth activation function across relu
    def __init__(self):
        super().__init__()
...
# def gem(x: Tensor, p: float | Tensor = 3., eps: float= 1e-6):
#     return F.avg_pool1d(x.clamp(min=eps).pow(p), (x.size(-1))).pow(1. / p)

class GeM(nn.Module):  # Generalized Mean Pooling, Pooling is to choose a value to represent the region by aggregating the values in the region
    def __init__(self, p: float | Tensor = 3., eps: float = 1e-6):
        super().__init__()
    ...
    def forward(self, x: Tensor):
        return F.avg_pool1d(
            x.clamp(min=self.eps).pow(self.p), (x.size(-1))
            ).pow(1. / self.p)
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            mask: Optional additive mask of shape compatible with attention matrix
            attn_mask: Optional multiplicative mask for attention

        Returns:
            output: Attention output of shape [batch_size, n_heads, seq_len_q, d_v]
            attn: Attention weights of shape [batch_size, n_heads, seq_len_q, seq_len_k]
        '''
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if mask is not None: mask = mask  # For head axis broadcasting
